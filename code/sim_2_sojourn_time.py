...
class N_Queue_single_station:
    ...
    def __init__(self,  sim_time, num_stations, services, arrivals_norm):
...
for sample in range(5000):
    ...
    if True:
        ...
        for trails in tqdm(range(2)):
            ...
            inp_steady_0 = np.concatenate((np.log(moms_arrive), np.log(moms_ser), np.array([1])))
        ...
        file_name =  'rho_' + str(rho)[:5] + '_num_servers_' + str(1) + '_sim_time_' + str(sim_time) + 'steady_' + str(
            model_num) + '.pkl'
