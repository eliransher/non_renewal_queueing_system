...
path = '/scratch/eliransc/non_renewal/steady_0'
file_name_used = []
...
path_dump_data_depart_0 = '/scratch/eliransc/non_renewal/training_batches/steady_0'
...
for batch_num in tqdm(range(len(os.listdir(path_dump_data_depart_0)), num_batches)):
    ...
    batch_name = 'steady_0_from_'+cluster_name+'_batch_num_' + str(batch_num)+'.pkl'

    pkl.dump((input_depart_0, output_depart_0), open(os.path.join(path_dump_data_depart_0, batch_name), 'wb'))
    pkl.dump(file_name_used, open('/scratch/eliransc/non_renewal/file_used_steady_0.pkl', 'wb'))