...
for sample in range(5000):
    ...
    arrivals_exp = np.random.exponential(1, 500000)
    moms_arrive = np.array(arrive_moms) # arrivals[2]
    ...
    sers_exp = np.random.exponential(1, 500000)
    services_norm =  sers_exp/ rate
    ...
    ser_moms = []
    for mom in range(1, 11):
        ser_moms.append(factorial(mom) / rate ** mom)

    moms_ser = np.array(ser_moms)
    ...
    for trails in tqdm(range(2)):
        ...
        n_Queue_single_station = N_Queue_single_station(sim_time, num_stations, services_norm, arrivals_exp,
                                                        num_servers)
    ...
    file_name =  'exp_rho_' + str(rho)[:5] + '_num_servers_' + str(num_servers) + '_sim_time_' + str(sim_time) + 'steady_' + str(
        model_num) + '.pkl'
