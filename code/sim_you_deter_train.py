...
for sample in range(1):
    if True:
        ...
        print('start sim')
        n_Queue_single_station = N_Queue_single_station(lamda, mu, sim_time, num_stations, services_times,
                                                        arrivals_norm)
        ...
        if sim_train:
            ...
            depart_0_moms = [(np.array(n_Queue_single_station.inter_departures[station]) ** mom).mean() for mom in
                             range(1, 11)]
            ...
            inp_depart_1 = np.concatenate(
                (np.log(np.array(depart_0_moms)), np.array(corrs_0), np.log(np.array(moms_ser[1]))))
